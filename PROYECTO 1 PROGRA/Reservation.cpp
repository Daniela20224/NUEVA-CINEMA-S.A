...
// Función showMenu sin inicialización estilo constructor
void Reservation::showMenu(const Maintenance& maintenance) {
    this->maintenance = maintenance;  // Asignación directa si maintenance no es referencia
    int option;
...
void Reservation::makeReservation() {
    ...
    int size = this->maintenance.movies.size();  // Asumiendo que tienes un getter
    int idMovie;
    ...
    // Validar que la posición está dentro de los límites (asumiendo matriz 8x8)
